...
bool HelloWorld::createBox2dWorld(bool debug)
{
	...
	// 바디를 생성해서 필드에 추가한다.
	this->AddNewSprite(Vec2(240, 160), Size(32, 32), b2_dynamicBody, NULL, 1);
...
void HelloWorld::onTouchMoved(Touch * touch, Event * event)
{
	...
	for (b2Body* b = _world->GetBodyList(); b; b = b->GetNext())
	{
		...
		{
			b->ApplyForce(b2Vec2(0, 1), b2Vec2(b->GetPosition().x, b->GetPosition().y + 10), true);
		}
	}
...
void HelloWorld::tick(float t)
{
	...
	for (b2Body *b = _world->GetBodyList(); b; b = b->GetNext())
	{
		...
		// 원은 userData가 NULL이므로 따로 확인해야한다.
		else if(b->GetUserData() == NULL && isForeced)
				b->ApplyForce(b2Vec2(0, 100), b2Vec2(b->GetPosition().x, b->GetPosition().y), true);
	}
}
